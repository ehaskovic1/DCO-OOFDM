...
%qammod rucno generisan
%----------------------------------------------------------
if log2(M) ~= round(log2(M))
    error('M mora biti stepen broja 2.');
end

if mod(length(data), k) ~= 0
    error('Broj bita u data mora biti djeljiv sa log2(M).');
end
data_reshape = reshape(data, k, []).';

symbols = bi2de(data_reshape, 'left-msb');

I = 2 * mod(symbols, sqrt(M)) - sqrt(M) + 1;
Q = 2 * floor(symbols / sqrt(M)) - sqrt(M) + 1;

constellation = (I + 1i * Q) / sqrt(mean(abs(I + 1i * Q).^2));

qam_modulisani = constellation;
...
qam_demodulated_symbols = zeros(size(primljeni_clipped));
for i = 1:length(primljeni_clipped)
    [~, idx] = min(abs(primljeni_clipped(i) - idealna_konstelacija));
    qam_demodulated_symbols(i) = idx - 1;
end

qam_demodulated_data = de2bi(qam_demodulated_symbols, log2(M), 'left-msb');
qam_demodulated_data = qam_demodulated_data(:).';
